...
"""
Download PMC OA bulk packages listed in an "Index of ..." HTML.

Features:
- Parse local HTML (saved "Index of ..." page)
- Filter targets: *.tar.gz and *.filelist.(csv|txt)
- Optional filter: baseline / incr
- Choose format: txt or xml (switch base URL automatically)
...
# txt (default base URL inferred from --format)
python dl_pmc_bulk.py --html pmc-oa_comm_txt.html --outdir ./pmc_txt --format txt

# xml (default base URL inferred from --format)
python dl_pmc_bulk.py --html pmc-oa_comm_xml.html --outdir ./pmc_xml --format xml

# override base URL explicitly (if you want a different OA set / path)
python dl_pmc_bulk.py --html index.html --outdir ./out --format xml \
  --base-url https://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/oa_comm/xml/
"""
...
DEFAULT_BASE_URL_TXT = "https://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/oa_comm/txt/"
DEFAULT_BASE_URL_XML = "https://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/oa_comm/xml/"
...
def href_to_url(href: str, base_url: str) -> Optional[str]:
    ...
    # skip parent directory and some unrelated links
    # (Index pages often include these)
    if href in ("../", "./"):
        return None
...
def resolve_base_url(fmt: str, base_url_arg: str) -> str:
    """
    If --base-url is specified, use it.
    Else decide by --format (txt/xml).
    """
    if base_url_arg:
        return base_url_arg
    if fmt == "xml":
        return DEFAULT_BASE_URL_XML
    return DEFAULT_BASE_URL_TXT


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--html", type=str, required=True, help="local index html, e.g., pmc-oa_comm_txt.html")
    ...
    ap.add_argument(
        ...
        default="txt",
        choices=["txt", "xml"],
        help="choose OA bulk format; default base URL follows this unless --base-url is provided",
    )
    ap.add_argument(
        "--base-url",
        type=str,
        default="",
        help="base URL for relative hrefs (overrides --format default)",
    )
    ...
    base_url = resolve_base_url(args.format, args.base_url)

    jobs = build_jobs(html_path, outdir, base_url, args.only)
    if not jobs:
